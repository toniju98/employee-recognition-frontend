...
export function useRewardSuggestions() {
  ...
  const createSuggestion = async (suggestion: { name: string; description: string }): Promise<boolean> => {
    try {
  ...
  const toggleVote = async (suggestionId: string): Promise<boolean> => {
    try {
  ...
  const reviewSuggestion = async (suggestionId: string, status: 'APPROVED' | 'REJECTED'): Promise<boolean> => {
    try {
