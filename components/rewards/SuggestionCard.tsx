...
export function SuggestionCard({ suggestion, onVote, reviewSuggestion }: SuggestionCardProps) {
  const { userInfo } = useAuth();
  const hasVoted = suggestion.votes.includes(userInfo?._id || '');
  const isAdmin = userInfo?.role === 'ADMIN';

  const handleSuggestions = async (status: "APPROVED" | "REJECTED") => {
    await reviewSuggestion?.(suggestion._id, status);
  };
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            {isAdmin && suggestion.status === "PENDING" && (
              <>
                <IconButton
                  onClick={() => handleSuggestions("APPROVED")}
                  color="success"
                  sx={{ p: 1 }}
                >
                ...
                <IconButton
                  onClick={() => handleSuggestions("REJECTED")}
                  color="error"
                  sx={{ p: 1 }}
                >
